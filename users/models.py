...
class Accountant(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        user, created = CustomUser.objects.get_or_create(email=self.email, first_name=self.first_name, last_name=self.last_name, is_accountant=self.isAccountant)
        if created:
            ...
            msg = "\nHey {} Welcome to {}, your username is {} and the default one time password is {}" \
                  ...
                  "This password is valid for 24 hours only".format((str(self.first_name) + str(self.last_name)), "this school ",self.email, user.password
                                                                    )
...
class Teacher(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None,
        ...
        # create a user with default password as firstname and lastname
        user, created = CustomUser.objects.get_or_create(email=self.email, first_name=self.first_name, last_name=self.last_name, is_teacher=self.isTeacher)
        if created:
            password = ("Complex."+str(self.empId[4:]))
            user.set_password(password)
            user.save()
